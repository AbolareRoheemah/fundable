...
mod PaymentStream {
    ...
    enum Event {
        ...
        StreamCreatedAndDeposited: StreamCreatedAndDeposited,
        StreamRestartedAndDeposited: StreamRestartedAndDeposited,
    ...
    #[derive(Drop, starknet::Event)]
    struct StreamCreatedAndDeposited {
        #[key]
        stream_id: u256,
        sender: ContractAddress,
        recipient: ContractAddress,
    ...
    #[derive(Drop, starknet::Event)]
    struct StreamRestartedAndDeposited {
        #[key]
        stream_id: u256,
        rate_per_second: UFixedPoint123x128,
    ...
    impl PaymentStreamImpl of IPaymentStream<ContractState> {
        ...
        fn create_stream_with_deposit(
            ...
            // Create stream with zero initial amount
            let stream_id = self.next_stream_id.read();
            self.next_stream_id.write(stream_id + 1);
        
            let duration = end_time - start_time;
            assert(duration >= 1, TOO_SHORT_DURATION);
            let rate_per_second = self.calculate_stream_rate(total_amount, duration);  // Still uses total_amount for rate
            ...
            self.accesscontrol._grant_role(STREAM_ADMIN_ROLE, sender);
            self.streams.write(stream_id, stream);
            self.erc721.mint(recipient, stream_id);
        
            // Use internal deposit function to handle funding
            ...
            self.emit(StreamCreatedAndDeposited {
                stream_id,
                sender,
                recipient,
                total_amount,
        ...
        fn restart_and_deposit(
            ...
            // deposit using the internal deposit function
            self._deposit(stream_id, amount);self._deposit(stream_id, amount);
            // Re-read stream after deposit to get updated total_amount
            stream = self.streams.read(stream_id);
