...
#[test]
fn test_create_stream_with_deposit_success() {
    ...
    let stream_id = payment_stream.create_stream_with_deposit(
        recipient,
        total_amount,
        start_time,
        end_time,
    ...
    // Assertions
    assert(stream_id == 0_u256, 'Stream ID should be 0');
    let stream = payment_stream.get_stream(stream_id);
    assert(stream.sender == sender, 'Sender mismatch');
    assert(stream.total_amount == total_amount, 'Total amount mismatch');
    assert(stream.balance == 0, 'Balance should be 0'); // _deposit doesn’t update balance
    assert(stream.start_time == start_time, 'Start time mismatch');
    assert(stream.end_time == end_time, 'End time mismatch');
    assert(stream.cancelable == cancelable, 'Cancelable mismatch');
    assert(stream.status == StreamStatus::Active, 'Status mismatch');

    let owner = erc721.owner_of(stream_id);
    assert(owner == recipient, 'NFT not minted to recipient');

    let metrics = payment_stream.get_protocol_metrics();
    assert(metrics.total_active_streams == 1, 'Active streams mismatch');
    assert(metrics.total_streams_created == 1, 'Streams created mismatch');
    assert(metrics.total_tokens_distributed == total_amount, 'Tokens distributed mismatch');
}

#[test]
#[should_panic(expected: ('INVALID_RECIPIENT',))]
fn test_create_stream_with_deposit_zero_recipient() {
    ...
    payment_stream.create_stream_with_deposit(
        contract_address_const::<0>(), total_amount, start_time, end_time, true, token_address, true
    );
    stop_cheat_caller_address(payment_stream.contract_address);
}
...
fn test_create_stream_with_deposit_insufficient_allowance() {
    ...
    start_cheat_caller_address(payment_stream.contract_address, sender);
    payment_stream.create_stream_with_deposit(
        recipient, total_amount, start_time, end_time, true, token_address, true
    );
    stop_cheat_caller_address(payment_stream.contract_address);
}

#[test]
#[should_panic(expected: ('TOO_SHORT_DURATION',))]
fn test_create_stream_with_deposit_too_short_duration() {
    let (token_address, sender, payment_stream, _) = setup();
    let recipient = contract_address_const::<'recipient'>();
    let total_amount = 1000_u256;
    let start_time = 100_u64;
    let end_time = 100_u64; // Same as start_time
...
// New tests for restart_and_deposit
#[test]
fn test_restart_and_deposit_success() {
    let (token_address, sender, payment_stream, _) = setup();
    let recipient = contract_address_const::<'recipient'>();
    let initial_amount = 1000_u256;
    let deposit_amount = 500_u256;
    let start_time = 100_u64;
    let end_time = 200_u64;
    let new_rate: UFixedPoint123x128 = 2_u256.into(); // 2 tokens/sec

    // Create and pause a stream
    start_cheat_caller_address(payment_stream.contract_address, sender);
    let stream_id = payment_stream.create_stream(
        recipient, initial_amount, start_time, end_time, true, token_address
    );
    ...
    // Restart and deposit
    start_cheat_caller_address(payment_stream.contract_address, sender);
    let success = payment_stream.restart_and_deposit(stream_id, new_rate, deposit_amount);
    stop_cheat_caller_address(payment_stream.contract_address);
    ...
    let stream = payment_stream.get_stream(stream_id);
    assert(stream.status == StreamStatus::Active, 'Status mismatch');
    assert(stream.rate_per_second == new_rate, 'Rate mismatch');
    assert(stream.total_amount == initial_amount + deposit_amount, 'Total amount mismatch');

    let metrics = payment_stream.get_protocol_metrics();
    assert(
        metrics.total_tokens_distributed == initial_amount + deposit_amount,
        'Tokens distributed mismatch'
    );
}

#[test]
#[should_panic(expected: ('Stream is not paused',))]
fn test_restart_and_deposit_not_paused() {
    let (token_address, sender, payment_stream, _) = setup();
    let recipient = contract_address_const::<'recipient'>();
    let initial_amount = 1000_u256;
    let deposit_amount = 500_u256;
    ...
    // Try to restart an active stream
    start_cheat_caller_address(payment_stream.contract_address, sender);
    payment_stream.restart_and_deposit(stream_id, 1_u256.into(), deposit_amount);
    stop_cheat_caller_address(payment_stream.contract_address);
}

#[test]
#[should_panic(expected: ('WRONG_SENDER',))]
fn test_restart_and_deposit_wrong_sender() {
    let (token_address, sender, payment_stream, _) = setup();
    let recipient = contract_address_const::<'recipient'>();
    ...
    start_cheat_caller_address(token_address, unauthorized);
    let token_dispatcher = IERC20Dispatcher { contract_address: token_address };
    token_dispatcher.approve(payment_stream.contract_address, deposit_amount);
    stop_cheat_caller_address(token_address);

    // Try to restart as unauthorized sender
...
fn test_restart_and_deposit_zero_rate() {
    let (token_address, sender, payment_stream, _) = setup();
    let recipient = contract_address_const::<'recipient'>();
    let initial_amount = 1000_u256;
    let deposit_amount = 500_u256;
    ...
    // Try to restart with zero rate
    start_cheat_caller_address(payment_stream.contract_address, sender);
    payment_stream.restart_and_deposit(stream_id, 0_u256.into(), deposit_amount);
    stop_cheat_caller_address(payment_stream.contract_address);
}
